...
def main():
    ...
    L = [
        ...
        "verify_quote(判词溯源核验)两个反幻觉工具。公开测试 Key 即刻可用(每 IP 每天 20 次),免费版 1,000 次/月,开发者 Pro ¥99/月 5 万次,"
        f"接入指南见 {BASE}/mcp/quickstart/。",
        f"- [私有化部署]({BASE}/#onpremise):面向大型律所与地方政府的离线部署,"
        "数据不出内网,可叠加本所历史卷宗做微调,权限按组织结构定制。",
        "",
        f"- [定价]({BASE}/pricing/):MCP/API 明码标价——公开测试 Key 免费(每 IP 每天 20 次,不用申请)、免费版 ¥0(1,000 次/月+100 篇全文)、"
